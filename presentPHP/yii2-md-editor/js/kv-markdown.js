...
function markUp(btn, source) {
    var el = $(source)
    var sel = el.getSelection(); //console.log(sel.start)
    var caretStart = -1;var caretEnd = -1;
    el.focus();  
    ...
    if (btn == 1) {
        ...
        if (str == '****') {caretStart = sel.start + 2; caretEnd = caretStart;}; 
    }
    // Italic  -- Revised for PS
    else if (btn == 2) {
        str = (txt.length > 0) ? getBlockMarkUp(txt, '*', '*') : '**';// (italic text here)
        if (str == '**') {caretStart = sel.start + 1; caretEnd = caretStart;}; 
    ...
    // Header -- Revised for PS
    else if (btn == 101) {
        // var pad = "#".repeat(headLevel); // "\n"+"=".repeat(len)
        txt = txt.trim("\n");
        str = (txt.length > 0) ? getMarkUp(txt, '', '\n==========\n') : '\n==========\n';
        if (str == '\n==========\n') {caretStart = sel.start; caretEnd = caretStart;}
    }
    // Higher Header -- Revised for PS
    else if (btn > 101 ) {
        var headerToken = "#".repeat(btn-100);
        str = getMarkUp(txt,headerToken+" ", " "+headerToken);
        caretStart = sel.start+btn-100+1; caretEnd = caretStart;
    }
    ...
    // Code Block -- Revised for PS
    else if (btn == 15) {
        str = getBlockMarkUp(txt, "    ", "\n");
        caretStart = sel.start+4; caretEnd = caretStart;
    ...
    // SetCaretPosition -- Revised for PS
    if (len ==0 && caretStart!=-1) {
        el.setSelection(caretStart,caretEnd);
    };
}
