...
def object2json():
	...
	FILES_FOLDER = os.path.join(PROJECT_ROOT, 'API/')
...
supported_paradigm = json2object()
...
def parser(md_plain, paradigm):
	...
	if supported_paradigm.has_key(paradigm): # if js_name in implemented:
		...
		html_parsed = prettify(html_parsed)
...
def md2xml_tree(raw_md,paradigm):#ani_type
	...
	# cut the content into slides	
		...
		# print '=test=', ET.tostring(body_tr, encoding='utf8', method='xml') , '\n ====='
		new_body_tr = con_list(body_tr)
		# print '=test=', ET.tostring(new_body_tr, encoding='utf8', method='xml') , '\n ====='
	else: 
...
if __name__ == "__main__":
	...
	try:
		...
		# print  "ERROR"
		result['ERROR'] = "Json Loads Exception"
		args = ['Error-Titile','Error-Content','S5']
		# sys.exit(1)
	...
	result['can-output'] = can_output
	result['html-parsed'] = html_parsed
